...
## [1.0.6c] — 2026-08-24 (HOTFIX — preview crash, wallet selector, balance staleness, reports & caching — continued)
...
- **BUG 1 — Wallet selector fails/delays in Transaction form (critical, 1–2 min hang)** — `transaction/new.tsx` and `[id].tsx` did `api.wallets()` on every `useFocusEffect` with no cache, no error handling, and no background revalidation. With 14 wallets, `GET /api/wallets` does FX conversion per wallet (sequential `await FxService.convert` with 8s external FX timeout on cold cache), so every modal open re-fetched slowly and any transient failure left the chip list empty forever with an infinite "Loading wallets..." and no retry. Root cause: redundant isolated fetch instead of shared cache, plus silent promise rejection and unbounded loading state. Fix: introduced `src/hooks/use-wallets.ts` module-level cache with stale-while-revalidate (instant cache hit, background refresh after 30s, deduplicated in-flight request), `walletsLoading`/`walletsError` with `console.error`, inline loading/error with **Retry**, and `invalidateWalletsCache()` after successful `createTransaction` so the next Wallets focus sees fresh balances immediately. Verified for Income, Expense, Transfer and for tab switches within the same form session. Backend `GET /api/wallets` also parallelized with `asyncio.gather` to cut cold-cache latency from sequential 14×8s to ~1×8s.
- **BUG 2 — Wallets tab floating spinner & slow fetch (~30s, unanchored over BRI card)** — `app/(tabs)/wallets.tsx` showed stale wallets while refetching with no anchored loading UI; `load()` on `useFocusEffect` only set `refreshing` on manual pull, so the auto-refetch after navigation appeared as a stray overlay spinner over the card list with no skeleton. Root cause: no loading/error state for focus refetch, silent `catch {}`, and `RefreshControl` not triggered for auto-refetch. Fix: migrated `wallets.tsx` to `useWallets` cache (instant cache hit, skeletons on initial load, `RefreshControl` properly anchored to `ScrollView` for refetch), added inline error with **Retry**, and ensured the backend parallelization speeds up the underlying fetch. Added `useFocusEffect(() => refresh())` to ensure cache invalidation after transaction is reflected on next focus.
- **BUG 3 — Reports ignores custom Start Date (resets to Aug 1)** — `Reports` used `useFocusEffect` for `load()` that depended on `fromDate`/`toDate`, so changing "From" while already focused never re-fetched until the screen lost and regained focus; the UI appeared to reset to month-start and the API was not re-queried with the new `from_date`. Root cause: `useFocusEffect` does not re-run on dependency change while focused; missing `useEffect` for date changes. Fix: added `useEffect(() => { load(); }, [load])` alongside `useFocusEffect` so date picker changes trigger an immediate `GET /api/transactions?from_date&to_date` (backend already supports `from_date`/`to_date` since 1.0.5c), plus `console.error` logging. Verified the API query string now correctly preserves the user-selected `fromDate`.
- **BUG 4 — System-wide slow fetches & lack of global caching (Home, Wallets, Plan, Reports, Coach)** — Every tab navigation triggered full uncached `api.wallets()` / `api.transactions` etc., causing 10–30s delays. Root cause: no shared fetch/caching layer; each screen did its own `fetch` on focus. Fix: introduced `useWallets` as the first shared cache (stale-while-revalidate, 30s, deduped) for the hottest path (wallets, used by 5+ screens); backend `GET /api/wallets` parallelized and `GET /api/transactions` already supports server-side date filtering to avoid over-fetching 500 recents. This eliminates redundant wallet fetches on transaction modal open and cuts Wallets/Reports latency. Full TanStack Query migration is the next step for other resources but out of scope for this hotfix.
- **Transactions "No transactions" vs Home recent, slow submit (>5s), stale Home recent, navigation not instant (follow-up)** — `app/transactions.tsx` did uncached `api.transactions()` with no loading/error state, so it showed "No transactions" while `Home` (which has `HOME_CACHE_KEY` persistent cache) showed data; `createTransaction` invalidated only wallets cache, so `Home` recent and `Transactions` list stayed stale for 30s until next background refresh. Root cause: no shared transaction cache, no invalidation of `Home`'s transaction state, and missing compound indexes on `wallets.id` + `transactions.date`. Fix: introduced `src/hooks/use-transactions.ts` (same stale-while-revalidate pattern as wallets, 200 limit, deduped), migrated `Transactions` screen to it with proper loading/error/Retry and `useFocusEffect` refresh, made `NewTransaction` invalidate both `wallets` and `transactions` caches plus `HOME_CACHE_KEY` (`storage.removeItem`) so `Home` recent updates immediately on next focus without 5s wait. Backend added compound indexes (`wallets: user_id+id`, `transactions: user_id+date`, `user_id+id`, `user_id+wallet_id`) to cut query latency that contributed to the >5s submit-to-visible delay.
