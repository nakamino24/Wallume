...
## [1.0.6c] — 2026-08-24 (HOTFIX — preview crash, wallet selector, balance staleness, reports & caching)
...
- **BUG 2 — Wallets tab floating spinner & slow fetch (~30s, unanchored over BRI card)** — `app/(tabs)/wallets.tsx` showed stale wallets while refetching with no anchored loading UI; `load()` on `useFocusEffect` only set `refreshing` on manual pull, so the auto-refetch after navigation appeared as a stray overlay spinner over the card list with no skeleton. Root cause: no loading/error state for focus refetch, silent `catch {}`, and `RefreshControl` not triggered for auto-refetch. Fix: migrated `wallets.tsx` to `useWallets` cache (instant cache hit, skeletons on initial load, `RefreshControl` properly anchored to `ScrollView` for refetch), added inline error with **Retry**, and ensured the backend parallelization speeds up the underlying fetch.
