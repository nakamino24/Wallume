...
- **Wallet Activity shows dates only**: the per-row time ("21:32") reflected when a record was *saved*, not anything the user entered — the form captures only a date. Removed from activity rows; the date remains visible via the group headers (TODAY / YESTERDAY / formatted day). Ordering logic is untouched: rows still sort by the real `created_at` with deterministic tie-breaking. The now-unused time formatter was removed rather than left as dead code. Audited all other surfaces — the global transactions list, Home recent list, and transaction edit screen never displayed times; the only other clock on screen (home-widget "updated at") is sync metadata, not transaction data.
