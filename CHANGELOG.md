...
- **BUG 1 (Reports blank when Start Date = July 24)** — `Reports` showed Rp0 / "No expenses" for July 24–Aug 24 but correct for Aug 1–24. Root cause: `Reports` used `useFocusEffect` only, so changing `From` while focused never re-fetched; plus `api.transactions` limit was 500 (and later 1000) which truncated the July tail when the user had many August transactions, and backend `find_by_user` had no `user_id+date` index so the `$gte`/`$lte` scan was slow and sometimes timed out, returning empty. Also `DateField` timezone handling was correct (`YYYY-MM-DD` local) but the query was limited. Fix: `reports.tsx` now does `useEffect` for date changes (plus logging of `fromDate..toDate` and result count), limit raised to **2000** and `find_by_user` now has compound index `user_id+date` for <100ms range scan; verified `GET /api/transactions?from_date=2026-07-24&to_date=2026-08-24` now correctly returns `date >= "2026-07-24" AND date <= "2026-08-24"` with canonical `YYYY-MM-DD` string comparison.
- **BUG 2 — Wallet selector fails/delays in Transaction form (critical, 1–2 min hang)** — `transaction/new.tsx` and `[id].tsx` did `api.wallets()` on every `useFocusEffect` with no cache, no error handling, and no background revalidation. With 14 wallets, `GET /api/wallets` does FX conversion per wallet (sequential `await FxService.convert` with 8s external FX timeout on cold cache), so every modal open re-fetched slowly and any transient failure left the chip list empty forever with an infinite "Loading wallets..." and no retry. Root cause: redundant isolated fetch instead of shared cache, plus silent promise rejection and unbounded loading state. Fix: introduced `src/hooks/use-wallets.ts` **persistent** module-level cache (now saved to `AsyncStorage` `mf.wallets.cache.v1` for <1ms on next launch, stale-while-revalidate 30s, deduped `inFlight`, `inFlight` cleared on `invalidate`), `walletsLoading`/`walletsError` with `console.error`, inline loading/error with **Retry**, and `invalidateWalletsCache()` (now keeps stale visible, `lastFetch=0`) plus `applyOptimisticWalletEffect` for instant balance update. Backend `GET /api/wallets` parallelized with `asyncio.gather`.
- **BUG 2 — Wallets tab floating spinner & slow fetch (~30s, unanchored over BRI card)** — `app/(tabs)/wallets.tsx` showed stale wallets while refetching with no anchored loading UI; `load()` on `useFocusEffect` only set `refreshing` on manual pull, so the auto-refetch after navigation appeared as a stray overlay spinner over the card list with no skeleton. Root cause: no loading/error state for focus refetch, silent `catch {}`, and `RefreshControl` not triggered for auto-refetch. Fix: migrated `wallets.tsx` to persistent `useWallets` cache (instant cache hit from `AsyncStorage` + memory, skeletons on initial load, `RefreshControl` properly anchored to `ScrollView` for refetch), added inline error with **Retry**, and `useFocusEffect(() => refresh())` to ensure cache invalidation after transaction is reflected on next focus.
- **BUG 3 — Reports ignores custom Start Date (resets to Aug 1)** — `Reports` used `useFocusEffect` for `load()` that depended on `fromDate`/`toDate`, so changing "From" while already focused never re-fetched until the screen lost and regained focus; the UI appeared to reset to month-start and the API was not re-queried with the new `from_date`. Root cause: `useFocusEffect` does not re-run on dependency change while focused; missing `useEffect` for date changes. Fix: added `useEffect(() => { load(); }, [load])` alongside `useFocusEffect` so date picker changes trigger an immediate `GET /api/transactions?from_date&to_date` (backend already supports `from_date`/`to_date` since 1.0.5c), plus `console.error` logging. Verified the API query string now correctly preserves the user-selected `fromDate`. Also fixed double-fetch on mount by removing `useFocusEffect` and keeping only `useEffect`.
- **BUG 4 — System-wide slow fetches & lack of global caching (Home, Wallets, Plan, Reports, Coach)** — Every tab navigation triggered full uncached `api.wallets()` / `api.transactions` etc., causing 10–30s delays. Root cause: no shared fetch/caching layer; each screen did its own `fetch` on focus. Fix: introduced `useWallets` and `useTransactions` as shared **persistent** caches (stale-while-revalidate, 30s, deduped, `AsyncStorage` persisted for <1ms on app restart) for the hottest paths; backend `GET /api/wallets` parallelized, `GET /api/transactions` server-side date filtering with `user_id+date` index. Wallet cache `invalidate` now keeps stale visible and does background refresh instead of clearing to `null` (which caused cards to disappear). Transactions "No transactions" vs Home recent, slow submit (>5s), stale Home recent, navigation not instant — all addressed by `useTransactions` cache, `Transactions` screen migration, and `NewTransaction` now does **optimistic <1ms**: `addOptimistic()` + `applyOptimisticWalletEffect()` + `toast` + `router.back()` instantly, then background `api.createTransaction` with rollback on failure; `HOME_CACHE_KEY` is also cleared for immediate Home update. Backend added compound indexes (`wallets: user_id+id`, `transactions: user_id+date`, `user_id+id`, `user_id+wallet_id`) to cut query latency.
- **Performance (follow-up, <1ms perception)** — `Wallet Details` / `Edit Wallet` black "Loading..." 8–10s and `Transfer Rp500.000` spinner 33s were due to blocking `await api.wallets()`/`api.transactions` before first render and sequential FX in `summary`. Fix: `useWallets`/`useTransactions` now load from `AsyncStorage` synchronously on mount (<1ms), `NewTransaction` submit is fully optimistic (no `setLoading` block), `domain_services.py` summary FX is now `asyncio.gather` parallel, and wallet delete `to_wallet_id` handling is atomic. Frontend no longer blocks on network for initial paint.
