...
import { FlexWidget, TextWidget, IconWidget, type HexColor } from 'react-native-android-widget';
...
export const WIDGET_EYE_ICON = '\uF2EA';
export const WIDGET_EYE_OFF_ICON = '\uF2E8';
...
export function NetWorthWidget({ data, large, small }: { data: NetWorthWidgetData; large?: boolean; small?: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Balance — eye isolated, balance content opens app */}
      <FlexWidget style={{ width: 'match_parent', marginTop: 4 }}>
        ...
        <FlexWidget style={{ width: 'match_parent', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <FlexWidget style={{ flex: 1 }} clickAction="OPEN_APP">
          ...
          <FlexWidget
            ...
            clickAction="TOGGLE_WIDGET_BALANCE"
            accessibilityLabel={id ? 'Tampilkan atau sembunyikan saldo widget' : 'Show or hide widget balance'}
          >
            <IconWidget icon={data.isBalanceVisible === true ? WIDGET_EYE_ICON : WIDGET_EYE_OFF_ICON} font="Ionicons" size={20} style={{ color: WHITE }} />
          </FlexWidget>
        </FlexWidget>
      </FlexWidget>

      {/* Monthly metrics — month label now clearly above them */}
      {!small && (
        <FlexWidget style={{ width: 'match_parent', marginTop: 6 }} clickAction="OPEN_APP">
          <TextWidget text={labels.month} style={{ color: isDark ? '#B6BDCB' : '#5E6C68', fontSize: 10 }} />
      ...
      {/* Financial Health — standalone, no chart */}
      {large && (
        <FlexWidget style={{ width: 'match_parent', marginTop: 8 }} clickAction="OPEN_APP">
